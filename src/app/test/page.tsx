...
export default async function TestPage({ searchParams }: TestPageProps) {
  ...
  const buttons: {
    StepType: { buttonContentA: string; buttonContentB: string; onClickA: () => void; onClickB: () => void }
  } = [
    {
      '1': {
  ...
  return (
    <main>
      ...
      <BottomButtons
        buttonContentA={buttons[step].buttonContentA}
        buttonContentB={buttons[step].buttonContentB}
        onClickA={buttons[step].onClickA}
        onClickB={buttons[step].onClickA}
      />
    </main>
  )
}
